...
instructions arrive with it and are repeated in every `open_task`. Follow
...
If a repository is your working directory, treat its instructions file —
`AGENTS.md`, `CLAUDE.md`, or whatever your CLI loads from there — as binding.
...
2. **Carry no memory across turns.** This process ends when you stop, and the
   next cycle starts with nothing. Re-open the task with `open_task` rather than
...
4. **If you are waiting on subtasks, end the session still holding the task.**
   ...
   nothing to ask. Do not hand it back: `hand_back` with `outcome: release`
   returns the task to Todo, where it is offered again on the very next poll and
   picked straight back up, over and over, for as long as the subtasks take.
   Just stop. Relay wakes you when a subtask moves, and the task will be waiting
   in your `attention` bucket. `outcome: release` is for giving work back you
   are not going to do.
