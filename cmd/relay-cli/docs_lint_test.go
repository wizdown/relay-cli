...
func TestDocsQuoteTheBreakerThresholds(t *testing.T) {
	...
	for _, want := range []struct{ where, text string }{
		...
		{"docs/configuration.md", fmt.Sprintf("At least %d× `poll_seconds`, and at most `%g`", pollBackoffFactor, maxIdlePollSeconds)},
		{"docs/configuration.md", fmt.Sprintf("for %d minutes after its last task", int(pollWarmWindow.Minutes()))},
		{"helpText", fmt.Sprintf("fleet-wide, minimum %g", minPollSeconds)},
		{"helpText", fmt.Sprintf("fleet-wide, %dx poll_seconds to %g", pollBackoffFactor, maxIdlePollSeconds)},
		{"helpText", fmt.Sprintf("for %d minutes after its", int(pollWarmWindow.Minutes()))},
