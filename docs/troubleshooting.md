...
| `paused by owner in relay …` | Resume the agent in Relay. This worker picks it up on its next poll. |
| `this agent is not in relay` | The probe breaker paused the worker. Put a live agent's credential in `relay_mcp`, then remove the `PAUSED` file. |
