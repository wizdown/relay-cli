...
| `the installed claude is not signed in` | Nothing started, because every claude worker would have failed in the same second. Run `claude auth login` as the user this fleet runs as. |
| `the installed codex is not signed in` | The same, for codex: run `codex login` once as this user and sign in with your ChatGPT account. relay-cli sets no API key and never copies those credentials. |
| `Not logged in · Please run /login` in `worker.log` | The sign-in lapsed *after* startup, so the check passed and the run did not. Log in again and restart. |
| `could not ask <cli> whether it is signed in` | A warning, not a failure: that CLI is too old to answer, or would not run. The start continues and a real failure surfaces on the first run. Upgrading the CLI restores the check. |
| `reports it is NOT signed in, but X is set — starting anyway` | The sign-in check stood down for that CLI: the environment carries a credential, which authenticates a run whatever the stored sign-in says. relay-cli trusts that the variable is set, not that it is valid. If every run then fails immediately, the key is wrong or left over from something unrelated — sign in properly and unset it. |
...
| `runtime_config.model is "…" — it takes one of` | The name is not one this build knows, on either runtime. Fix the typo, use one of the aliases the error lists, or — if the model is newer than this relay-cli — set `RELAY_CLI_SKIP_MODEL_CHECK=1`. See [A model this build has not heard of](configuration.md#a-model-this-build-has-not-heard-of). |
| A worker's logs name a model you did not write | You wrote an alias, and relay-cli pins each one to an id — `sonnet` runs `claude-sonnet-5`. That is what stops the model moving under a config nobody edited: [Aliases are pinned, not tracked](configuration.md#aliases-are-pinned-not-tracked). |
| `is not one runtime "codex" offered when this relay-cli was built` | A warning, not a failure: the model check stood down because `RELAY_CLI_SKIP_MODEL_CHECK` or `RELAY_CLI_SKIP_RUNTIME_CHECK` is set, and the name went to the CLI unchecked. If every run then fails immediately, it was a typo after all. |
...
| `no coding CLI found on PATH, and a worker is one` | `relay init` refused, because a worker drives a CLI this binary does not bundle. Install [Claude Code](https://claude.com/claude-code) or the [Codex CLI](https://developers.openai.com/codex/cli), sign it in, and run `init` again — nothing was written, so there is no file to move aside. |
| A worker for the runtime you wanted is commented out | That CLI was not on `PATH` when `relay init` ran. Install and sign it in, then delete the `// ` from those lines — the comma that joins it to the worker above is already inside the comment. |
...
| `runtime "claude" is unusable` / `runtime "codex" is unusable` | The CLI is missing from `PATH`, too old for the flags the adapter needs, or not signed in. The error names which. See [Runtimes](runtimes.md#the-startup-check). |
